...
//metoda care calculeaza distanta dintre 2 puncte de pe harta
int PathFinder::distanceManhatan(int x1, int y1, int x2, int y2) {
    return abs(x1 - x2) + abs(y1 - y2);
}

//metoda care verifica daca agentul poate sa traverseze celula respectiva sau nu
bool PathFinder::canMoveTo(int x, int y, const Map& map, bool canFly) {
    ...
    return map.getCellRole(x, y) != WALL;
}

//converteste coordonatele pt hash map
...
std::vector<Position> PathFinder::getPath(
    ...
    int startKey = coordToKey(startX, startY, map.getCols());
    ...
    while (!nodesList.empty()) {
        ...
        if (x < 0 || x >= map.getRows() || y < 0 || y >= map.getCols()) {
            if (visited[x][y]) {
                continue;
            }
        }
        ...
        for (int i = 0; i< 4; i++) {
            int vecinX = dx[i] + x;
            int vecinY = dy[i] + y;
            int newKey = coordToKey(vecinX, vecinY, map.getCols());

            //veridficam daca vecinul e valid si nu a fost visitat
            if (vecinX >= 0 && vecinX < map.getRows() &&
                vecinY >= 0 && vecinY < map.getCols() &&
    ...
    if (pathFound) {
        ...
        //facem drumul inapoi, de la destinatie la punctul de plecare
        while (!(x == startX &&y == startY)) {
            path.push_back(Position(x, y));

            int key = coordToKey(x, y, map.getCols());
...
//metoda gaseste si returneaza drumul catre cea mai apropiata statie de incarcare
Position PathFinder::findNearestStation(const Map &map, Agent *agent) {
    ...
    //verifica pt toate statiile care este mai apropiata
