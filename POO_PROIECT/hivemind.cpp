...
void Hivemind::allocatePackages() {
    ...
    while (true) {
        ...
        if (!packageToAssign) break;

        Agent* bestAgent = nullptr;
        int minDist = INT_MAX;

        for (auto agent: agents) {
            if (agent->isDead() || agent->countAssignedPackages() > 0) continue;

            Position agentPos = agent->getAgentPosition();
            Position destPos = packageToAssign->getDestPosition();
            int dist = abs(agentPos.getPositionX() - destPos.getPositionX()) +
                       abs(agentPos.getPositionY() - destPos.getPositionY());
